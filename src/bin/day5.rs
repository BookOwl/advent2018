...
fn react_polymer(poly: &str, pat: &regex::Regex) -> String {
...
fn part1() {
    ...
    let poly = react_polymer(POLYMER, &pat);
...
fn part2() {
    ...
    for (unitp, unitn) in [('a', 'A'), ('b', 'B'), ('c', 'C'), ('d', 'D'), ('e', 'E'), ('f', 'F'), 
                           ...
                           ('y', 'Y'), ('z', 'Z')].iter() {
        let poly = POLYMER.replace(*unitn, "").replace(*unitp, "");
        let reacted = react_polymer(&poly, &pat);
